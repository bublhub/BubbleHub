...
function Set-BubbleHubUtf8NoBomContent {
    param(
        [string]$Path,
        [string]$Value
    )
...
function Invoke-WslBash {
    ...
    Set-BubbleHubUtf8NoBomContent -Path $TempScript -Value $Command
...
`$Encoding = New-Object System.Text.UTF8Encoding `$false
[System.IO.File]::WriteAllText(`$TempCommand, `$Command, `$Encoding)
