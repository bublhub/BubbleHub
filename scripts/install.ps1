...
$Repo = if ($env:BUBBLEHUB_REPO) { $env:BUBBLEHUB_REPO } else { "bubblehub-labs/bubblehub-runtime" }
...
function New-BubbleHubControlCenterShortcut {
    param(
...
function Install-BubbleHubWindowsLaunchers {
...
'@ | Set-Content -Path $CmdLauncher -Encoding ASCII
    ...
    if ($InstallDesktopShortcut) {
        ...
        foreach ($ShortcutPath in $ShortcutPaths) {
            New-BubbleHubControlCenterShortcut `
                -Shell $Shell `
...
function Assert-BubbleHubWslReady {
...
if ($env:OS -eq "Windows_NT") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Install-BubbleHubWindowsLaunchers -InstallDesktopShortcut:$DesktopInstalled
    }
    exit $LASTEXITCODE
}
