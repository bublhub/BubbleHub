...
int ageos_apply_cgroup_limits(const ageos_sandbox_config *cfg) {
    if (cfg == NULL) {
        return -EINVAL;
    }
    char dir[256];
    snprintf(dir, sizeof(dir), "/sys/fs/cgroup/ageos/%ld", (long)getpid());
    if (mkdir("/sys/fs/cgroup/ageos", 0755) != 0 && errno != EEXIST) {
