...
from bubblehub.app.agents import normalize_agent_name, write_agent_metadata
from bubblehub.inference import apply_inference_env
...
def command(
    ...
    """Run a binary as an BubbleHub agent inside the hardened sandbox."""
...
def run_agent(
    *,
    ...
    if rootfs_dir is not None and sandbox_paths.host_root_dir is None:
        overlay_workspace_dir = tempfile.TemporaryDirectory(prefix="bubblehub-workspace-")
    ...
    env = dict()
    env["BUBBLEHUB_AGENT_ID"] = agent_id
    if agent_name is not None:
        env["BUBBLEHUB_AGENT_NAME"] = agent_name
    env["BUBBLEHUB_NICENESS"] = str(niceness)
    env["BUBBLEHUB_HOST_UID"] = str(os.getuid())
    env["BUBBLEHUB_HOST_GID"] = str(os.getgid())
    # we want to see logs for the sandbox invocation, we will remove these later for the agent
    env["BUBBLEHUB_LOG_FILE"] = os.environ.get("BUBBLEHUB_LOG_FILE", "")
    env["BUBBLEHUB_LOG_LEVEL"] = os.environ.get("BUBBLEHUB_LOG_LEVEL", "")
    env["BUBBLEHUB_ENABLE_SECCOMP"] = os.environ.get("BUBBLEHUB_ENABLE_SECCOMP", "")
    for state_key in ("BUBBLEHUB_STATE_DIR", "XDG_STATE_HOME", "HOME", "BUBBLEHUB_SANDBOX_HELPER", "LD_LIBRARY_PATH"):
        if os.environ.get(state_key):
            env[state_key] = os.environ[state_key]
    _prepare_access_manifest_dir(agent_id, env)
    bubblehub_pythonpath = _bubblehub_site_packages()
    if bubblehub_pythonpath is not None:
        env["BUBBLEHUB_PYTHONPATH"] = str(bubblehub_pythonpath)
    if rootfs_dir is not None:
        env["BUBBLEHUB_ROOTFS_RELEASE"] = _ROOTFS_RELEASE
    endpoint = apply_inference_env(env, speciality)
    log_info("using inference endpoint", endpoint)
    typer.echo(f"Using BubbleHub inference endpoint at {endpoint}")
...
def _find_persistent_agent_id(root: Path) -> str | None:
    bubblehub_dir = root / _BUBBLEHUB_DIR
    agents_dir = bubblehub_dir / _AGENTS_DIR
    marker = bubblehub_dir / _CURRENT_AGENT_FILE
    if marker.is_file():
...
def _record_persistent_sandbox(root_dir: str | None, agent_id: str) -> None:
    if root_dir is None:
        return
    bubblehub_dir = Path(root_dir) / _BUBBLEHUB_DIR
    bubblehub_dir.mkdir(mode=0o700, exist_ok=True)
    (bubblehub_dir / _CURRENT_AGENT_FILE).write_text(f"{agent_id}\n", encoding="utf-8")


def _remove_persistent_agent(root: Path, agent_id: str) -> None:
    agents_dir = root / _BUBBLEHUB_DIR / _AGENTS_DIR
    ...
    if resolved_agent.parent != resolved_agents:
        raise typer.BadParameter("persistent sandbox path escaped .bubblehub/agents")
    try:
...
def _resolve_rootfs_dir() -> Path | None:
    if os.environ.get("BUBBLEHUB_DISABLE_ROOTFS") == "1":
        return None
    configured = os.environ.get("BUBBLEHUB_ROOTFS_DIR")
    rootfs = Path(configured).expanduser() if configured else _DEFAULT_ROOTFS_DIR
    if not rootfs.exists():
        if configured:
            raise typer.BadParameter(f"BubbleHub rootfs not found: {rootfs}")
        return None
    if not rootfs.is_dir():
        raise typer.BadParameter(f"BubbleHub rootfs is not a directory: {rootfs}")
...
def _prepare_overlay_paths(root_dir: str | None, agent_id: str, rootfs_dir: Path | None) -> OverlayPaths | None:
    ...
    agent_dir = Path(root_dir) / _BUBBLEHUB_DIR / _AGENTS_DIR / agent_id
...
def _prepare_sandbox_binary(
    ...
    staging_dir = tempfile.TemporaryDirectory(prefix="bubblehub-workspace-")
...
def _argv_for_binary(binary: Path | str) -> list[str]:
    ...
    if binary_path.suffix == ".py":
        return [_bubblehub_python(), binary_text]
    return [binary_text]
...
def _bubblehub_site_packages() -> Path | None:
    candidates: list[Path] = []
    install_prefix = Path(os.environ.get("BUBBLEHUB_PREFIX", "/opt/bubblehub"))
...
def _access_state_root(env: Mapping[str, str]) -> Path:
    if env.get("BUBBLEHUB_STATE_DIR"):
        return Path(env["BUBBLEHUB_STATE_DIR"]).expanduser()
    if env.get("XDG_STATE_HOME"):
        return Path(env["XDG_STATE_HOME"]).expanduser() / "bubblehub"
    if env.get("HOME"):
        return Path(env["HOME"]).expanduser() / ".local" / "state" / "bubblehub"
    return Path(f"/tmp/bubblehub-{os.getuid()}") / "state"
...
def _choose_access_policy(
    ...
    *,
    ...
    output_stream.write(
        "\nBubbleHub sandbox paused for host access decision:\n"
...
def _source_checkout_root() -> Path | None:
    ...
    if (candidate / "pyproject.toml").exists() and (candidate / "bubblehub").is_dir():
        return candidate
    return None


def _bubblehub_source_tree_for(root: Path) -> Path | None:
...
def _apply_sandbox_inference_env(env: dict[str, str], endpoint: SandboxInferenceEndpoint) -> None:
    env["BUBBLEHUB_API_BASE_URL"] = endpoint.sandbox_base_url
    env["OPENAI_BASE_URL"] = f"{endpoint.sandbox_base_url}/v1"
    env["BUBBLEHUB_SANDBOX_INFERENCE_HOST"] = "127.0.0.1"
    env["BUBBLEHUB_SANDBOX_INFERENCE_PORT"] = str(endpoint.sandbox_port)
...
def _apply_sandbox_http_proxy_env(env: dict[str, str], proxy_port: int) -> None:
    ...
    env["http_proxy"] = proxy_url
    env["https_proxy"] = proxy_url
    env["BUBBLEHUB_HTTP_PROXY_PORT"] = str(proxy_port)
