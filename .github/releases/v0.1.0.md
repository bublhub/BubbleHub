...
* Added hardened agent execution through `libbubblehub`, including native scheduler bindings, sandbox launch support, hardware detection, and filesystem/network isolation.
...
* Refactored the native source tree from `c` to `libbubblehub` for clearer project structure and packaging paths.
...
* Added source-tree protection so agents cannot run with the BubbleHub application source as their writable workspace.
...
```text
https://bubblehub.dev/download/latest
```

Specific release asset base:
```text
https://bubblehub.dev/download/v0.1.0
```
...
```bash
curl -LO https://bubblehub.dev/download/latest/BubbleHub-0.1.0-x64.deb
...
```powershell
irm https://bubblehub.dev/download/latest/BubbleHub-0.1.0-x64.exe -OutFile BubbleHub-0.1.0-x64.exe
.\BubbleHub-0.1.0-x64.exe
```

## 🔗 Links

🌐 Website → [BubbleHub Labs](https://bubblehub-labs.com)
💻 GitHub → [bubblehub-runtime](https://github.com/bubblehub-labs/bubblehub-runtime)
