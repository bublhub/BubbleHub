<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/bubblehub-logo-dark.png">
    <img src="assets/bubblehub-logo.png" alt="BubbleHub logo" width="175">
  </picture>
  <p>Local LLM serving and sandboxed agents in one command.</p>
  <p>
    <a href="https://github.com/bubblehub-labs/bubblehub-runtime/actions/workflows/ci.yml"><img alt="CI" src="https://github.com/bubblehub-labs/bubblehub-runtime/actions/workflows/ci.yml/badge.svg"></a>
    <a href="https://codecov.io/gh/bubblehub-labs/bubblehub-runtime"><img alt="Coverage" src="https://codecov.io/gh/bubblehub-labs/bubblehub-runtime/graph/badge.svg"></a>
    <a href="https://github.com/bubblehub-labs/bubblehub-runtime/releases/latest"><img alt="GitHub release" src="https://img.shields.io/github/v/release/bubblehub-labs/bubblehub-runtime?display_name=tag"></a>
...
```bash
curl -fsSL https://bubblehub.dev/install.sh | bash
```

Windows PowerShell, through WSL:

```powershell
irm https://bubblehub.dev/install.ps1 | iex
```
...
```bash
docker pull ghcr.io/bubblehub-labs/bubblehub-runtime:latest
```

Use a release image as a base:

```dockerfile
FROM ghcr.io/bubblehub-labs/bubblehub-runtime:v0.1.0
```
...
```bash
curl -fsSL https://bubblehub.dev/download/linux/v0.1.0/install.sh | BUBBLEHUB_VERSION=v0.1.0 bash
```

Download a specific Debian package:

```bash
curl -LO https://bubblehub.dev/download/linux/v0.1.0/BubbleHub-0.1.0-x64.deb
...
```bash
docker pull ghcr.io/bubblehub-labs/bubblehub-runtime:v0.1.0
```
## Build from source
...
```bash
docker build -f docker/Dockerfile --target unit-test -t bubblehub-runtime:unit .
docker run --rm --privileged --security-opt seccomp=unconfined bubblehub-runtime:unit
```

CI enforces line coverage through [Codecov](https://codecov.io/gh/bubblehub-labs/bubblehub-runtime) (45% project target for `libbubblehub` and `bubblehub`). See [CONTRIBUTING.md](CONTRIBUTING.md#coverage) for the local coverage command and HTML report locations.
...
docker build -f docker/Dockerfile --target integration-test -t bubblehub-runtime:integration .
