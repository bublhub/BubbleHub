...
#define OVERFS_LOG_ERRNO(text, fmt, ...) \
    AGEOS_LOG_ERROR(text, fmt " err=%s", ##__VA_ARGS__, strerror(errno))
...
int ageos_overfs_mkdir_p(const char *path, mode_t mode) {
    if (path == NULL || path[0] == '\0') {
        AGEOS_LOG_ERROR("invalid overfs directory path", "path=%s", path != NULL ? path : "(null)");
        return -EINVAL;
    }
    ...
    AGEOS_LOG_DEBUG("ensured overfs directory", "path=%s mode=%o", buffer, mode);
    return 0;
}
...
static int ensure_parent_dir(const char *path, mode_t mode) {
    ...
    return ageos_overfs_mkdir_p(parent, mode);
}

int ageos_overfs_ensure_file(const char *path, mode_t mode) {
    struct stat st;
    if (stat(path, &st) == 0) {
        AGEOS_LOG_DEBUG("overfs file already exists", "path=%s regular=%d", path, S_ISREG(st.st_mode));
    ...
    close(fd);
    AGEOS_LOG_DEBUG("created overfs file", "path=%s mode=%o", path, mode);
    return 0;
}

int ageos_overfs_bind_file_readonly(const char *source, const char *target) {
    AGEOS_LOG_DEBUG("binding overfs file readonly", "source=%s target=%s", source, target);
...
int ageos_overfs_bind_file_readwrite(const char *source, const char *target) {
    AGEOS_LOG_DEBUG("binding overfs file readwrite", "source=%s target=%s", source, target);
    if (mount(source, target, NULL, MS_BIND, NULL) != 0) {
...
int ageos_overfs_bind_dir(const char *source, const char *target) {
    AGEOS_LOG_DEBUG("binding overfs directory", "source=%s target=%s", source, target);
    if (mount(source, target, NULL, MS_BIND, NULL) != 0) {
...
static int bind_dir_readonly(const char *source, const char *target) {
    AGEOS_LOG_DEBUG("binding overfs directory readonly", "source=%s target=%s", source, target);
...
int ageos_overfs_bind_optional_dir_readonly(const char *source, const char *target) {
    ...
    if (stat(source, &st) != 0) {
        if (errno == ENOENT) {
            AGEOS_LOG_DEBUG("skipping missing optional overfs directory bind", "source=%s target=%s", source, target);
        } else {
    ...
    if (!S_ISDIR(st.st_mode)) {
        AGEOS_LOG_DEBUG("skipping non-directory optional overfs bind", "source=%s target=%s", source, target);
        return 0;
    }
    int rc = ageos_overfs_mkdir_p(target, 0755);
    if (rc != 0) {
        return rc;
    }
...
int ageos_overfs_bind_optional_file_readonly(const char *source, const char *target) {
    ...
    if (stat(source, &st) != 0) {
        if (errno == ENOENT) {
            AGEOS_LOG_DEBUG("skipping missing optional overfs file bind", "source=%s target=%s", source, target);
        } else {
    ...
    if (!S_ISREG(st.st_mode)) {
        AGEOS_LOG_DEBUG("skipping non-file optional overfs bind", "source=%s target=%s", source, target);
        return 0;
    }
    int rc = ageos_overfs_ensure_file(target, 0644);
    if (rc != 0) {
        return rc;
    }
    return ageos_overfs_bind_file_readonly(source, target);
}
...
static int bind_optional_node_readwrite(const char *source, const char *target) {
    ...
    if (stat(source, &st) != 0) {
        if (errno == ENOENT) {
            AGEOS_LOG_DEBUG("skipping missing optional overfs node bind", "source=%s target=%s", source, target);
        } else {
    ...
    if (!S_ISCHR(st.st_mode) && !S_ISBLK(st.st_mode) && !S_ISFIFO(st.st_mode)) {
        AGEOS_LOG_DEBUG("skipping non-node optional overfs bind", "source=%s target=%s", source, target);
        return 0;
    }
    int rc = ageos_overfs_ensure_file(target, 0666);
    if (rc != 0) {
        return rc;
    }
    return ageos_overfs_bind_file_readwrite(source, target);
}

int ageos_overfs_mount_tmpfs_at(const char *target, const char *options) {
    AGEOS_LOG_DEBUG("mounting overfs tmpfs", "target=%s options=%s", target, options != NULL ? options : "");
    int rc = ageos_overfs_mkdir_p(target, 0755);
    if (rc != 0) {
        return rc;
    }
    if (mount("tmpfs", target, "tmpfs", MS_NOSUID | MS_NODEV, options) != 0) {
        if (errno == EBUSY) {
            AGEOS_LOG_DEBUG("overfs tmpfs already mounted", "target=%s", target);
        } else {
...
static int setup_device_mounts(const char *new_root) {
    AGEOS_LOG_DEBUG("setting up overfs device mounts", "root=%s", new_root);
    char dev_path[PATH_MAX];
    int rc = ageos_overfs_join_mount_path(new_root, "/dev", dev_path, sizeof(dev_path));
    if (rc != 0) {
        return rc;
    }
    rc = ageos_overfs_mkdir_p(dev_path, 0755);
    if (rc != 0) {
        return rc;
    }
    ...
    AGEOS_LOG_DEBUG("mounted overfs dev tmpfs", "target=%s", dev_path);
    const char *devices[] = {
        "/dev/null",
        "/dev/zero",
        "/dev/random",
    ...
    for (size_t i = 0; i < sizeof(devices) / sizeof(devices[0]); i++) {
        ...
        rc = ageos_overfs_join_mount_path(new_root, devices[i], target, sizeof(target));
        if (rc != 0) {
            return rc;
        }
...
static int setup_proc_mount(const char *new_root) {
    AGEOS_LOG_DEBUG("setting up overfs host proc bind", "root=%s", new_root);
    char proc_path[PATH_MAX];
    int rc = ageos_overfs_join_mount_path(new_root, "/proc", proc_path, sizeof(proc_path));
    if (rc != 0) {
        return rc;
    }
    rc = ageos_overfs_mkdir_p(proc_path, 0555);
    if (rc != 0) {
        return rc;
    }
    if (mount("/proc", proc_path, NULL, MS_BIND | MS_REC, NULL) != 0) {
        if (errno == EBUSY) {
            AGEOS_LOG_DEBUG("overfs host proc already mounted", "target=%s", proc_path);
            return 0;
        }
        OVERFS_LOG_ERRNO("failed to bind overfs host proc", "target=%s", proc_path);
        return -errno;
    }
    AGEOS_LOG_INFO("bound host proc into overfs root", "target=%s", proc_path);
...
static int setup_ageos_runtime_binds(const char *new_root) {
    AGEOS_LOG_DEBUG("setting up overfs AgeOS runtime binds", "root=%s", new_root);
    struct {
        const char *source;
        const char *target;
    } dirs[] = {
    ...
    for (size_t i = 0; i < sizeof(dirs) / sizeof(dirs[0]); i++) {
        char target[PATH_MAX];
        int rc = ageos_overfs_join_mount_path(new_root, dirs[i].target, target, sizeof(target));
        if (rc != 0) {
            return rc;
        }
        rc = ageos_overfs_bind_optional_dir_readonly(dirs[i].source, target);
        if (rc != 0) {
            return rc;
        }
    }
    ...
    const char *files[] = {
        "/usr/bin/ageos",
        "/usr/bin/ageos-node",
        "/usr/bin/ageos-sandbox",
        "/usr/bin/llama-server",
        "/usr/lib/libageos.so",
        "/usr/lib/x86_64-linux-gnu/libageos.so",
    };
    for (size_t i = 0; i < sizeof(files) / sizeof(files[0]); i++) {
        char target[PATH_MAX];
        int rc = ageos_overfs_join_mount_path(new_root, files[i], target, sizeof(target));
        if (rc != 0) {
            return rc;
        }
        rc = ageos_overfs_bind_optional_file_readonly(files[i], target);
        if (rc != 0) {
            return rc;
        }
    }
...
static int setup_overlay_root(const ageos_sandbox_config *cfg, const char *new_root) {
    AGEOS_LOG_INFO(
        "setting up AgeOS overfs root",
    ...
    rc = ageos_overfs_mkdir_p(cfg->overlay_work_dir, 0700);
    if (rc != 0) {
        return rc;
    }
    ...
    if (mount("overlay", new_root, "overlay", MS_NOSUID | MS_NODEV, options) != 0) {
        int err = errno;
        AGEOS_LOG_ERROR(
            "failed to mount AgeOS overfs overlay",
    ...
    AGEOS_LOG_INFO("mounted AgeOS overfs overlay", "root=%s", new_root);
    free(options);

    char tmp_path[PATH_MAX];
    rc = ageos_overfs_join_mount_path(new_root, "/tmp", tmp_path, sizeof(tmp_path));
    if (rc != 0) {
        return rc;
    }
    rc = ageos_overfs_mount_tmpfs_at(tmp_path, "mode=1777,size=512m");
    if (rc != 0) {
        return rc;
    }
    ...
    return setup_ageos_runtime_binds(new_root);
}

int ageos_overfs_setup_mounts(const char *new_root, const ageos_sandbox_config *cfg) {
    AGEOS_LOG_DEBUG("setting up overfs mounts", "root=%s rootfs_enabled=%d", new_root, ageos_overfs_rootfs_enabled(cfg));
    if (mount(NULL, "/", NULL, MS_REC | MS_PRIVATE, NULL) != 0) {
        OVERFS_LOG_ERRNO("failed to make mount namespace private", "root=%s", new_root);
        return -errno;
    }
    if (!ageos_overfs_rootfs_enabled(cfg)) {
        AGEOS_LOG_DEBUG("skipping AgeOS overfs root because no rootfs is configured", "root=%s", new_root);
        return 0;
    }
