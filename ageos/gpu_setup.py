...
def choose_install_profile(hardware: HardwareInfo, mode: str) -> dict[str, object]:
    ...
    if normalized not in GPU_MODES:
        return _profile(hardware, "cpu", False, False, f"unsupported AGEOS_GPU mode '{mode}'")
    if normalized == "cpu":
        return _profile(hardware, "cpu", False, True, "CPU mode forced by AGEOS_GPU=cpu")
    if normalized == "vllm":
        return _profile(
            hardware,
            "vllm",
    ...
    if normalized in LLAMA_GPU_BACKENDS:
        return _profile(
            ...
            f"{normalized} forced by AGEOS_GPU={normalized}",
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Configure optional AgeOS GPU dependencies.")
