...
def load_inference_config() -> InferenceConfig:
    ...
    if not isinstance(inference, dict):
        inference = {}
    host = os.environ.get("AGEOS_INFERENCE_HOST") or str(inference.get("host") or "127.0.0.1")
    port = _optional_int(os.environ.get("AGEOS_INFERENCE_PORT")) or _optional_int(inference.get("port")) or 8000
    default_specialty = (
        os.environ.get("AGEOS_INFERENCE_SPECIALTY")
        or os.environ.get("AGEOS_SPECIALITY")
...
def apply_inference_env(env: dict[str, str], specialty: str | None = None) -> str:
    ...
    env["AGEOS_API_BASE_URL"] = base_url
    env["OPENAI_BASE_URL"] = f"{base_url.rstrip('/')}/v1"
    env.setdefault("OPENAI_API_KEY", "ageos-local")
    env["AGEOS_SPECIALITY"] = specialty or config.default_specialty
