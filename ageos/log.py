...
def configure_logging(level: str | None = None, log_file: str | Path | None = None) -> str:
    """Configure AgeOS log verbosity and optional log file output."""

    global _current_level, _configured
    raw = (level or os.environ.get("AGEOS_LOG_LEVEL", "error")).strip().lower()
    if raw == "debug":
        resolved = "debug"
        ...
        resolved = "error"
        _current_level = _LogLevel.ERROR
    os.environ["AGEOS_LOG_LEVEL"] = resolved
    _configured = True
...
def _configure_log_file(log_file: str | Path | None) -> None:
    ...
    resolved = log_file
    if resolved is None:
        env_file = os.environ.get("AGEOS_LOG_FILE")
        if env_file:
            resolved = env_file

    if not resolved:
        os.environ.pop("AGEOS_LOG_FILE", None)
        _sync_native_logging(os.environ.get("AGEOS_LOG_LEVEL", "error"))
        return
    ...
    os.environ["AGEOS_LOG_FILE"] = str(path)
    _sync_native_logging(os.environ.get("AGEOS_LOG_LEVEL", "error"))
...
def _caller_location() -> tuple[str, int]:
    for frame_info in inspect.stack()[1:]:
        ...
        if path.name == "log.py" and path.parent.name == "ageos":
            continue
...
def _is_sandboxed() -> bool:
    if os.environ.get("AGEOS_SANDBOX") == "1":
        return True
    try:
...
def _sandbox_log_roots() -> list[Path]:
    roots: list[Path] = []
    for name in ("AGEOS_AGENT_HOME", "AGEOS_WORKSPACE", "TMPDIR", "HOME"):
...
def _sync_native_logging(level: str) -> None:
    try:
        from ageos.native import sync_log_config
