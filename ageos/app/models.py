...
def run_install_base_model_setup(
    ...
    *,
    ...
    explicit = os.environ.get("AGEOS_BASE_MODEL", "").strip()
    if explicit:
        select_model_for_speciality(speciality, explicit)
        return True

    if os.environ.get("AGEOS_SKIP_MODEL_SETUP", "").strip() == "1":
        return False
...
def prompt_base_model_setup(
    ...
    *,
    ...
    choice_index = choose_option(
        ...
        message=(f"Choose the default model for {speciality}.\n" "AgeOS will use this as your base instruct model on this machine."),
        options=options,
