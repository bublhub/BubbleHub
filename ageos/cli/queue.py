...
def command(
    ...
    """Show waiting AgeOS model/inference work."""
...
def _render(console: Console) -> None:
    table = Table(title="AgeOS Waiting Queue")
    ...
    if not queue:
        table.add_row("", "", "", "", "", "", "No waiting jobs; admitted work appears in ageos ps/dashboard models.")
        console.print(table)
        return
    for item in queue:
