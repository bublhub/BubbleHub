...
def test_install_ps1_routes_commands_to_configured_wsl_distro() -> None:
    ...
    assert '"bash", $WslScript' in script
    assert "bash `$WslCommandPath" in script
...
def test_windows_install_script_installs_release_deb_in_wsl() -> None:
    ...
    windows_start = script.index('if ($env:OS -eq "Windows_NT")')
    windows_branch = script[windows_start : script.index("$TempScript = New-TemporaryFile", windows_start)]
