...
from bubblehub.integrations.anthropic_shim import BubblehubAnthropic
...
def test_anthropic_shim_uses_native_session_even_when_api_base_is_set(monkeypatch) -> None:
    ...
    with patch("bubblehub.integrations.anthropic_shim.EngineSession") as session_cls:
        session = session_cls.return_value.__enter__.return_value
        session.chat.return_value = "native"
        response = BubblehubAnthropic(speciality="default-instruct", niceness=2).messages.create(
...
def test_langchain_wrapper_uses_native_session(monkeypatch) -> None:
    from bubblehub.integrations import langchain as langchain_module
    ...
    with patch("bubblehub.integrations.langchain.EngineSession") as session_cls:
        session = session_cls.return_value.__enter__.return_value
        session.chat.return_value = "native"
        model = langchain_module.BubblehubChatModel(speciality="default-instruct", niceness=4)
