...
def test_openai_shim_uses_native_session_even_when_api_base_is_set(monkeypatch) -> None:
    ...
    with patch("bubblehub.integrations.openai_shim.EngineSession") as session_cls:
        session = session_cls.return_value.__enter__.return_value
        session.chat.side_effect = ["first", "second"]
        client = BubblehubOpenAI(speciality="default-instruct")
        first = client.chat.completions.create(model="bubblehub-local", messages=[{"role": "user", "content": "one"}])
        second = client.chat.completions.create(model="bubblehub-local", messages=[{"role": "user", "content": "two"}])
...
def test_openai_shim_passes_max_tokens_to_native_session(monkeypatch) -> None:
    ...
    with patch("bubblehub.integrations.openai_shim.EngineSession") as session_cls:
        ...
        response = BubblehubOpenAI(speciality="default-instruct", niceness=3).chat.completions.create(
...
def test_openai_shim_forwards_to_sandbox_endpoint(monkeypatch) -> None:
    ...
    response = BubblehubOpenAI(speciality="default-instruct").chat.completions.create(
        model="bubblehub-local",
        messages=[{"role": "user", "content": "hi"}],
        max_tokens=16,
    )
...
def test_openai_shim_defaults_to_configured_speciality(monkeypatch) -> None:
    ...
    with (
        patch(
            "bubblehub.integrations.openai_shim.load_inference_config",
            return_value=SimpleNamespace(default_specialty="default-instruct"),
        ),
        patch("bubblehub.integrations.openai_shim.EngineSession") as session_cls,
    ):
        session = session_cls.return_value.__enter__.return_value
        session.chat.return_value = "direct"

        BubblehubOpenAI().chat.completions.create(
            model="bubblehub-local",
            messages=[{"role": "user", "content": "hi"}],
        )
